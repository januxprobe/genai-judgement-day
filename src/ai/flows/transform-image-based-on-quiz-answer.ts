 ...
 * and robots/characters should appear in the background scenery.
 *
...
const transformImageFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    let coreInstructions = `**TASK: EVOLVE THE USER'S BACKGROUND ADDITIVELY, INCLUDING THEMATIC ROBOTS/CHARACTERS**
...
`;
    ...
    - All such additions must be behind the user and enhance the scene without altering the user. Imagine you are adding a new layer of detail or a new set of characters/objects to an existing scene.`;
      console.log(`Using pre-generated reference description for '${themeName}' (Question ${input.questionNumber}). Guiding AI to add theme-appropriate background robots based on this and general theme.`);
    } else {
      ...
      if (themeName === 'Code') {
        ...
        coreInstructions += ` The background evolution for 'Chaos' should add glitchy, abstract, aggressive elements, possibly incorporating neon yellow (hex #04D9FF) accents. Think distorted digital artifacts, chaotic energy lines, or fragmented light effects appearing within the existing environment. Crucially, **add some fitting robots or automatons (e.g., damaged, rogue, unpredictable, or heavily armed types) into the background scenery.**`;
      }
      console.log(`No pre-generated reference description provided for '${themeName}' (Question ${input.questionNumber}). Using default theme description with explicit focus on adding background robots.`);
    }
...
*   **CHANGES ARE ADDITIVE TO THE BACKGROUND:** The goal is to evolve the background, not discard previous changes entirely. New thematic elements (including robots) should integrate with what's already there.
*   **ROBOTS/CHARACTERS ARE BACKGROUND ELEMENTS:** All robots, droids, or other characters suggested by the theme or description MUST be depicted as part of the background environment, not as a transformation of the user. For instance, show them walking in the distance, flying by, integrated into machinery, or as part of a crowd/army in the background. They should not interact directly with the preserved user.`;

    finalImagePromptParts.push({ text: coreInstructions });

    finalImagePromptParts.push({ text: "\n\n**THE USER PHOTO (Identify person, keep them 100% unchanged, EVOLVE their background additively based on all instructions above, ensuring any new robots/characters are in the background):**" });
...
2.  **Evolve their existing background** by additively incorporating new elements, styles, and theme-appropriate ROBOTS/CHARACTERS as described above, inspired by the '${themeName}' theme and the provided current step description/logic. 
3.  Ensure all generated elements, especially robots or other characters, ONLY appear in this evolving background, BEHIND the preserved person.
Provide a brief text description of the newly generated background, focusing on what new elements were added (including any robots/characters) and how they incorporate the theme and object/style references from the current step.`
    });
    ...
    if (!transformedPhotoDataUri) {
      ...
      transformationDescription = "AI image generation failed to return a new image. Displaying the previous image state. No new background robots were added due to this failure.";
